...
We will use the built-in data set from the **{migrate}** package

```{r mock_credit_data}
# Load package data
data(mock_credit)

# Check out the a few rows of data
mock_credit %>% 
...
Using the `migrate()` function, we can calculate the sum of **principal_balance** for each *starting* and *ending* **risk rating** combination
...
# Capture the unique summed *principal_balance* amounts representing risk state 
...
We wanted to first embed the hex logos for the **{migrate}** and **{gt}** packages as *clickable* links on either side of the title, so we first built the markdown strings that will be used 
...
Lastly, we can apply these hex logos some final formatting touches to the remainder of the table
